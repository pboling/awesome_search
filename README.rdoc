...
These are the defaults, and may be configured separately for each search class you create.
  ...
  # You need to set the variable that will contain the search results, results. results is set to [] in the super, so you can iterate and do << with it.
      self.results = Model.find(stuff) if stuff
      nil
    end
  end
